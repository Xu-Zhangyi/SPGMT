...
def get_train_label(input_dis_matrix, input_time_matrix, count):
    ...
    for i in tqdm(range(len(input_dis_matrix))):  # (5000,5000)
        ...
        if len(re_idx) > 20:
            ...
            label_dis.append(out_val[1:count+1])
            neg_label.append(re_idx[count+1:])
            neg_label_dis.append(out_val[count+1:])
        else:
            label.append(re_idx[1:count+1])
            label_dis.append(out_val[1:count+1])
            neg_label.append(re_idx[-count:-1])
...
train_time_matrix = np.load(osp.join('ground_truth', config.dataset,
                                     config.distance_type, 'train_temporal_distance.npy'))
vali_time_matrix = np.load(osp.join('ground_truth', config.dataset,
                                    config.distance_type, 'vali_temporal_distance.npy'))
test_time_matrix = np.load(osp.join('ground_truth', config.dataset,
...
if config.distance_type == 'LCRS':
    ...
    train_time_matrix[train_time_matrix == LCRS_time_num] = -1
# %==========================================================================================================

re_train_list, re_train_coor, re_train_d2vec, re_train_dis_matrix, re_train_time_matrix = re_matrix(
    train_list, train_coor, train_d2vec, train_dis_matrix, train_time_matrix, config.pos_num*2)
...
re_train_time_matrix[re_train_time_matrix < 0] = -1
...
# %==========================================================================================================
train_y, train_neg_y, train_dis, train_neg_dis = get_train_label(
